...
核心思路是**分层**。运输层主进程是ws服务器子进程是客户端或stdio，而rpc层子进程是服务器主进程是rpc转发路由；
```
┌─────────────────────────────────────────────────────────┐
│                         主进程                            │
├─────────────────────────────────────────────────────────┤
│  运载层: WebSocket Server (监听外部客户端)                │
│  职责: 连接管理、心跳保活、认证鉴权、协议适配              │
│                           ↓                              │
│  RPC层: RPC Client + 智能路由                            │
│  职责: 将WebSocket消息路由到子进程、负载均衡、故障转移     │
└─────────────────────────────────────────────────────────┘
                            ↕ (AF_UNIX双向通信)
┌─────────────────────────────────────────────────────────┐
│                         子进程                            │
├─────────────────────────────────────────────────────────┤
│  RPC层: RPC Server (监听Unix Socket)                     │
│  职责: 接收RPC请求、业务逻辑处理、会话状态管理              │
│                           ↓                              │
│  运载层: WebSocket Client (向上连接主进程)                │
│  职责: 主动注册、心跳上报、推送消息                       │
└─────────────────────────────────────────────────────────┘
```
